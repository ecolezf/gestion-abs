...
export function DirectorDashboard({
  ...
  useEffect(() => {
    async function loadData() {
      ...
      // -------------------------------------------------------
      // Affichage des éventuelles erreurs Supabase
      // -------------------------------------------------------
      if (levelsRes.error) {
        console.error(
      ...
      // Classes ayant au moins un enseignant
      const assignedClassIds = new Set(
        (ctRes.data || []).map(
          (ct: { class_id: string }) => ct.class_id
        )
      );

      const classesWithAssignment = allClasses.map((cls) => ({
        ...cls,
        hasTeachers: assignedClassIds.has(cls.id),
      }));
  ...
  // =========================================================
  // Classes affectées / non affectées
  // =========================================================
  const assignedClasses = classes.filter(
    (cls) => (cls as any).hasTeachers
  );
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        {stats.map((stat, index) => (
          ...
          >
            ...
            <div
              className={`w-10 h-10 rounded-xl ${stat.bg} ${stat.text} flex items-center justify-center mb-3`}
            >
              {stat.icon}
            </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          {quickActions.map((action, index) => (
            ...
            >
              ...
              <div
                className={`w-10 h-10 rounded-xl ${action.bg} ${action.text} flex items-center justify-center`}
              >
                {action.icon}
              </div>
